---
title: "Homework 5"
author: "[Yuchen Ma]{style='background-color: yellow;'}"
toc: true
title-block-banner: true
title-block-style: default
execute: 
  freeze: true
  cache: true
format:
  html: # comment this line to get pdf
  # pdf: 
    fig-width: 7
    fig-height: 7
---


---

::: {.callout-important style="font-size: 0.8em;"}

Please read the instructions carefully before submitting your assignment.

1. This assignment requires you to only upload a `PDF` file on Canvas
1. Don't collapse any code cells before submitting. 
1. Remember to make sure all your code output is rendered properly before uploading your submission.

⚠️ Please add your name to the author information in the frontmatter before submitting your assignment ⚠️
:::


In this assignment, we will explore decision trees, support vector machines and neural networks for classification and regression. The assignment is designed to test your ability to fit and analyze these models with different configurations and compare their performance.

We will need the following packages:


```{R, message=FALSE, warning=FALSE, results='hide'}
packages <- c(
  "tibble",
  "dplyr", 
  "readr", 
  "tidyr", 
  "purrr", 
  "broom",
  "magrittr",
  "corrplot",
  "caret",
  "rpart",
  "rpart.plot",
  "e1071",
  "torch", 
  "luz"
)

# renv::install(packages)
sapply(packages, require, character.only=T)
```

<br><br><br><br>
---

## Question 1
::: {.callout-tip}
## 60 points
Prediction of Median House prices
:::

###### 1.1 (2.5 points)


The `data` folder contains the `housing.csv` dataset which contains housing prices in California from the 1990 California census. The objective is to predict the median house price for California districts based on various features.


Read the data file as a tibble in R. Preprocess the data such that:

1. the variables are of the right data type, e.g., categorical variables are encoded as factors
2. all column names to lower case for consistency
3. Any observations with missing values are dropped

```{R}
library(tidyr)

path <- "data/housing.csv"

df <- read_csv(path) %>% 
  mutate(across(where(is.character), as.factor)) %>% 
  rename_with(tolower) %>% 
  drop_na() 
head(df)
```

---

###### 1.2 (2.5 points)

Visualize the correlation matrix of all numeric columns in `df` using `corrplot()`

```{R}
df %>%
  select(where(is.numeric)) %>% 
  cor() %>% 
  corrplot(method = "circle", type = "upper", order = "hclust", 
           tl.col = "black", tl.srt = 45, 
           addCoef.col = "black")
```

---

###### 1.3 (5 points)

Split the data `df` into `df_train` and `df_split` using `test_ind` in the code below:

```{R}
set.seed(42)
test_ind <- sample(
  1:nrow(df), 
  floor( nrow(df)/10 ),
  replace=FALSE
)

df_train <- df[-test_ind, ] 
df_test  <- df[test_ind, ] 
```

---

###### 1.4 (5 points)

Fit a linear regression model to predict the `median_house_value` :

* `latitude`
* `longitude`
* `housing_median_age`
* `total_rooms`
* `total_bedrooms`
* `population`
* `median_income`
* `ocean_proximity`

Interpret the coefficients and summarize your results. 

```{R}
lm_fit <- lm(median_house_value ~ latitude + longitude + housing_median_age + total_rooms + total_bedrooms + population + median_income + ocean_proximity, data = df_train)
summary(lm_fit)
```

---

###### 1.5 (5 points)

Complete the `rmse` function for computing the Root Mean-Squared Error between the true `y` and the predicted `yhat`, and use it to compute the RMSE for the regression model on `df_test`

```{R}
rmse <- function(y, yhat) {
  sqrt(mean((y - yhat)^2))
}

lm_predictions <- predict(lm_fit, df_test)
test_rmse <- rmse(df_test$median_house_value, lm_predictions)
print(test_rmse)
```


###### 1.6 (5 points)

Fit a decision tree model to predict the `median_house_value` using the same predictors as in 1.4. Use the `rpart()` function.

```{R}
rpart_fit <- rpart(median_house_value ~ latitude + longitude + housing_median_age + total_rooms + total_bedrooms + population + median_income + ocean_proximity, data = df_train, method = "anova")

rpart_predictions <- predict(rpart_fit, df_test)
```


Visualize the decision tree using the `rpart.plot()` function. 

```{R}
library(rpart.plot)
rpart.plot(rpart_fit, main="Decision Tree for Median House Value", under=TRUE, faclen=0)

```


Report the root mean squared error on the test set.

```{R}
test_rmse_rpart <- rmse(df_test$median_house_value, rpart_predictions)
print(test_rmse_rpart)
```

---

###### 1.7 (5 points)

Fit a support vector machine model to predict the `median_house_value` using the same predictors as in 1.4. Use the `svm()` function and use any kernel of your choice. Report the root mean squared error on the test set.

```{R}
svm_fit <- svm(median_house_value ~ ., data = df_train)
svm_predictions <- predict(svm_fit, newdata = df_test)

rmse <- function(actual, predicted) {
  sqrt(mean((actual - predicted)^2))
}

test_rmse_svm <- rmse(df_test$median_house_value, svm_predictions)
print(test_rmse_svm)
```


---

###### 1.8 (25 points)

Initialize a neural network model architecture:

```{R}
library(torch)
library(luz)

# Define the neural network architecture
NNet <- nn_module(
  initialize = function(p, q1 = 32, q2 = 16, q3 = 8){
    self$layer1 <- nn_linear(p, q1)
    self$relu1 <- nn_relu()
    self$layer2 <- nn_linear(q1, q2)
    self$relu2 <- nn_relu()
    self$layer3 <- nn_linear(q2, q3)
    self$relu3 <- nn_relu()
    self$output <- nn_linear(q3, 1) # Assuming the output layer has 1 node for regression
  },
  forward = function(x){
    x <- self$layer1(x)
    x <- self$relu1(x)
    x <- self$layer2(x)
    x <- self$relu2(x)
    x <- self$layer3(x)
    x <- self$relu3(x)
    x <- self$output(x)
    x
  }
)

# Prepare the data
df_train_matrix <- model.matrix(median_house_value ~ . - 1, data = df_train)
target_train <- df_train$median_house_value
df_test_matrix <- model.matrix(median_house_value ~ . - 1, data = df_test)
target_test <- df_test$median_house_value

train_dataset <- tensor_dataset(list(
  x = torch_tensor(df_train_matrix, dtype = torch::torch_float32()), 
  y = torch_tensor(target_train, dtype = torch::torch_float32())
))

test_dataset <- tensor_dataset(list(
  x = torch_tensor(df_test_matrix, dtype = torch::torch_float32()), 
  y = torch_tensor(target_test, dtype = torch::torch_float32())
))

train_dataloader <- dataloader(train_dataset, batch_size = 64, shuffle = TRUE)
test_dataloader <- dataloader(test_dataset, batch_size = 64, shuffle = FALSE)

```


Fit a neural network model to predict the `median_house_value` using the same predictors as in 1.4. Use the `model.matrix` function to create the covariate matrix and `luz` package for fitting the network with $32, 16, 8$ nodes in each of the three hidden layers. 


```{R}
# Load necessary libraries
library(torch)
library(luz)

# Assuming NNet is defined and instantiated properly...
model <- NNet(p = ncol(df_train_matrix))

# Define the optimizer and loss function
optimizer <- optim_adam(model$parameters, lr = 0.001)
loss_fn <- nn_mse_loss(reduction = 'mean')

rmse_metric <- luz_metric(
  initialize = function() {
    list(diff = torch_tensor(0, dtype = torch_float64()), n = 0)
  },
  update = function(state, output, target) {
    state$diff <- state$diff + torch_sum((output - target)^2)
    state$n <- state$n + target$size(1)
    state
  },
  finalize = function(state) {
    sqrt(state$diff$item() / state$n)
  }
)


#fit_results <- model %>%
  #setup(loss = loss_fn, optimizer = optimizer) %>%
  #fit(dataloader = train_dataloader, epochs = 100, valid_dataloader = test_dataloader)


```

Plot the results of the training and validation loss and accuracy.

```{R}
#plot(nnet_fit$metrics$epoch, nnet_fit$metrics$loss, type = 'l', col = 'blue', xlab = "Epoch", ylab = "Loss", main = "Training and Validation Loss")
#lines(nnet_fit$metrics$epoch, nnet_fit$metrics$val_loss, col = 'red')
#legend("topright", legend = c("Training", "Validation"), col = c("blue", "red"), lty = 1)

```


Report the root mean squared error on the test set.


```{R}
#nnet_predictions <- predict(nnet_fit, test_dataloader)
#nnet_predictions <- as_array(nnet_predictions)

#test_rmse_nnet <- rmse(df_test$median_house_value, nnet_predictions)
#print(test_rmse_nnet)

```

::: {.callout-warning}
Remember to use the `as_array()` function to convert the predictions to a vector of numbers before computing the RMSE with `rmse()`
:::

---

###### 1.9 (5 points)

Summarize your results in a table comparing the RMSE for the different models. Which model performed best? Why do you think that is?

```{R}
#rmse_linear_regression <- linear_regression_rmse_value
#rmse_decision_tree <- ecision_tree_rmse_value
#rmse_svm <- your_svm_rmse_value
#rmse_neural_network <- neural_network_rmse_value

#rmse_summary <- data.frame(
  #Model = c("Linear Regression", "Decision Tree", "SVM", "Neural Network"),
  #RMSE = c(rmse_linear_regression, rmse_decision_tree, rmse_svm, rmse_neural_network))


#rmse_summary <- rmse_summary[order(rmse_summary$RMSE), ]
#print(rmse_summary)
#best_model <- rmse_summary[which.min(rmse_summary$RMSE), ]
#cat("The best performing model is", best_model$Model, "with an RMSE of", best_model$RMSE, "\n")

```



<br><br><br><br>
<br><br><br><br>
---

## Question 2
::: {.callout-tip}
## 50 points
Spam email classification
:::

The `data` folder contains the `spam.csv` dataset. This dataset contains features extracted from a collection of spam and non-spam emails. The objective is to classify the emails as spam or non-spam.

---

###### 2.1 (2.5 points)

Read the data file as a tibble in R. Preprocess the data such that:

1. the variables are of the right data type, e.g., categorical variables are encoded as factors
2. all column names to lower case for consistency
3. Any observations with missing values are dropped

```{R}
library(tidyverse)
df2 <- read_csv("data/spambase.csv") %>%
  mutate(across(where(is.character), as.factor)) %>%
  rename_with(tolower) %>%
  drop_na()
head(df2)


```

---

###### 2.2 (2.5 points)

Split the data `df` into `df_train` and `df_split` using `test_ind` in the code below:

```{R}
set.seed(42)
test_ind <- sample(1:nrow(df2), floor(nrow(df2) / 10), replace = FALSE)

df_train <- df2[-test_ind, ]
df_test <- df2[test_ind, ]

head(df_train)
head(df_test)
```

Complete the `overview` function which returns a data frame with the following columns: `accuracy`, `error`, `false positive rate`, `true positive rate`, between the true `true_class` and the predicted `pred_class` for any classification model.

```{R}
overview <- function(pred_class, true_class) {
  confusion_matrix <- table(Predicted = pred_class, True = true_class)
  true_positives <- confusion_matrix[2, 2]
  true_negatives <- confusion_matrix[1, 1]
  false_positives <- confusion_matrix[1, 2]
  false_negatives <- confusion_matrix[2, 1]
  accuracy <- (true_positives + true_negatives) / sum(confusion_matrix)
  error <- 1 - accuracy
  true_positive_rate <- true_positives / (true_positives + false_negatives)
  false_positive_rate <- false_positives / (false_positives + true_negatives)
  return(data.frame(accuracy = accuracy, error = error, true_positive_rate = true_positive_rate, false_positive_rate = false_positive_rate))
}


```


---

###### 2.3 (5 points)

Fit a logistic regression model to predict the `spam` variable using the remaining predictors. Report the prediction accuracy on the test set.

```{R}
glm_fit <- glm(spam ~ ., data = df_train, family = binomial)

probabilities <- predict(glm_fit, df_test, type = "response")
glm_classes <- ifelse(probabilities > 0.5, 1, 0)

accuracy <- mean(glm_classes == df_test$spam)
print(accuracy)


```

---

###### 2.4 (5 points)

Fit a decision tree model to predict the `spam` variable using the remaining predictors. Use the `rpart()` function and set the `method` argument to `"class"`. 

```{R}
library(rpart)
library(rpart.plot)
rpart_fit <- rpart(spam ~ ., data = df_train, method = "class")
rpart_classes <- predict(rpart_fit, newdata = df_test, type = "class")
```

Visualize the decision tree using the `rpart.plot()` function. 

```{R}
rpart.plot(rpart_fit)
```

Report the prediction accuracy on the test set.

```{R}
accuracy <- mean(rpart_classes == df_test$spam)
print(accuracy)
```

---

###### 2.5 (5 points)

Fit a support vector machine model to predict the `spam` variable using the remaining predictors. Use the `svm()` function and use any kernel of your choice. Remember to set the `type` argument to `"C-classification"` **if you haven't** already converted `spam` to be of type `factor`.


```{R}
library(e1071)

svm_fit <- svm(spam ~ ., data = df_train, type = "C-classification", kernel = "radial")
```
Report the prediction accuracy on the test set.

```{R}
svm_classes <- predict(svm_fit, newdata = df_test)
accuracy <- mean(svm_classes == df_test$spam)
print(accuracy)
```

---

###### 2.6 (25 points)

Using the same neural network architecture as in 1.9, fit a neural network model to predict the `spam` variable using the remaining predictors. 

::: {.callout-warning}
## Classification vs. Regression

Note that the neural network in **Q 1.9** was a regression model. You will need to modify the neural network architecture to be a classification model by changing the output layer to have a single node with a sigmoid activation function.
:::

Use the `model.matrix` function to create the covariate matrix and `luz` package for fitting the network with $32, 16, 8$ nodes in each of the three hidden layers. 

```{R}
library(torch)
library(luz)
NNet <- nn_module(
  "NNet",
  initialize = function(num_features) {
    self$layer1 <- nn_linear(num_features, 32)
    self$relu1 <- nn_relu()
    self$layer2 <- nn_linear(32, 16)
    self$relu2 <- nn_relu()
    self$layer3 <- nn_linear(16, 8)
    self$relu3 <- nn_relu()
    self$output <- nn_linear(8, 1)
    self$sigmoid <- nn_sigmoid()
  },
  forward = function(x) {
    x %>% 
      self$layer1() %>%
      self$relu1() %>%
      self$layer2() %>%
      self$relu2() %>%
      self$layer3() %>%
      self$relu3() %>%
      self$output() %>%
      self$sigmoid()
  }
)

model <- NNet(ncol(df_train))
train_data <- model.matrix(~ . - 1, data = df_train)
train_labels <- df_train$spam
train_dataset <- tensor_dataset(
  x = torch_tensor(train_data, dtype = torch_float32()),
  y = torch_tensor(as.numeric(train_labels) - 1, dtype = torch_float32())
)
train_dl <- dataloader(train_dataset, batch_size = 64, shuffle = TRUE)

test_data <- model.matrix(~ . - 1, data = df_test)
test_labels <- df_test$spam
test_dataset <- tensor_dataset(
  x = torch_tensor(test_data, dtype = torch_float32()),
  y = torch_tensor(as.numeric(test_labels) - 1, dtype = torch_float32())
)
#test_dl <- dataloader(test_dataset, batch_size = 64)
#nnet_fit <- model %>% 
  #setup(
    #loss = nn_bce_loss(),
    #optimizer = optim_adam(model$parameters, lr = 0.001)
  #) %>%
  #set_hparams(
    #num_features = ncol(df_train)
  #) %>%
  #fit(
    #dataloader = train_dl,
    #epochs = 100,
    #valid_dataloader = test_dl,
    #verbose = FALSE
  #)

```

---

###### 2.7 (5 points)

Summarize your results in a table comparing the accuracy metrics for the different models. 

```{R}
#accuracy_glm <- ... # Accuracy of the logistic regression model
#accuracy_rpart <- ... # Accuracy of the decision tree model
#accuracy_svm <- ... # Accuracy of the SVM model
#accuracy_nnet <- ... # Accuracy of the neural network model
#model_accuracies <- data.frame(
  #Model = c("Logistic Regression", "Decision Tree", "SVM", "Neural Network"),
  #Accuracy = c(accuracy_glm, accuracy_rpart, accuracy_svm, accuracy_nnet)
#)

#print(model_accuracies)

```

If you were to choose a model to classify spam emails, which model would you choose? Think about the context of the problem and the cost of false positives and false negatives.


I would choose a model that balances high accuracy with low false positives and is computationally efficient, considering the high inconvenience cost of misclassifying legitimate emails as spam.

<br><br><br><br>
<br><br><br><br>
---

## Question 3
::: {.callout-tip}
## 60 points

Three spirals classification

:::

To better illustrate the power of depth in neural networks, we will use a toy dataset called the "Three Spirals" data. This dataset consists of two intertwined spirals, making it challenging for shallow models to classify the data accurately. 

::: {.callout-warning}
## This is a multi-class classification problem
:::

The dataset can be generated using the provided R code below:

```{R}
generate_three_spirals <- function(){
  set.seed(42)
  n <- 500
  noise <- 0.2
  t <- (1:n) / n * 2 * pi
  x1 <- c(
      t * (sin(t) + rnorm(n, 0, noise)),
      t * (sin(t + 2 * pi/3) + rnorm(n, 0, noise)),
      t * (sin(t + 4 * pi/3) + rnorm(n, 0, noise))
    )
  x2 <- c(
      t * (cos(t) + rnorm(n, 0, noise)),
      t * (cos(t + 2 * pi/3) + rnorm(n, 0, noise)),
      t * (cos(t + 4 * pi/3) + rnorm(n, 0, noise))
    )
  y <- as.factor(
    c(
      rep(0, n), 
      rep(1, n), 
      rep(2, n)
    )
  )
  return(tibble::tibble(x1=x1, x2=x2, y=y))
}
```

---

###### 3.1 (5 points)

Generate the three spirals dataset using the code above. Plot $x_1$ vs $x_2$ and use the `y` variable to color the points. 


```{R}
df <- generate_three_spirals()

plot(
  df$x1, df$x2,
  col = df$y,
  pch = 20
)
```

Define a grid of $100$ points from $-10$ to $10$ in both $x_1$ and $x_2$ using the `expand.grid()`. Save it as a tibble called `df_test`. 

```{R}
grid <- expand.grid(x1 = seq(-10, 10, length.out = 100), x2 = seq(-10, 10, length.out = 100))
df_test <- as_tibble(grid)

```

---

###### 3.2 (10 points)

Fit a classification tree model to predict the `y` variable using the `x1` and `x2` predictors, and plot the decision boundary. 

```{R}
library(rpart)
rpart_fit <- rpart(y ~ x1 + x2, data = df, method = "class")
rpart_classes <- predict(rpart_fit, newdata = df_test, type = "class")
```

Plot the decision boundary using the following function:

```{R}
plot_decision_boundary <- function(predictions){
  plot(
    df_test$x1, df_test$x2, 
    col = predictions,
    pch = 0
  )
  points(
    df$x1, df$x2,
    col = df$y,
    pch = 20
  )
}
```

```{R}
plot_decision_boundary(rpart_classes)
```

---

###### 3.3 (10 points)

Fit a support vector machine model to predict the `y` variable using the `x1` and `x2` predictors. Use the `svm()` function and use any kernel of your choice. Remember to set the `type` argument to `"C-classification"` **if you haven't** converted `y` to be of type `factor`.

```{R}
library(e1071)
svm_fit <- svm(y ~ x1 + x2, data = df, type = "C-classification", kernel = "radial")
svm_classes <- predict(svm_fit, newdata = df_test)
plot_decision_boundary(svm_classes)

```

---

::: {.callout-warning}
## Instructions

For the next questions, you will need to fit a series of neural networks. In all cases, you can:

* set the number of units in each hidden layer to 10 
* set the output dimension `o` to 3 (remember this is multinomial classification)
* use the appropriate loss function for the problem (**not `nn_bce_loss`**)
* set the number of epochs to $50$
* fit the model using the `luz` package

You can use any optimizer of your choice, but you **will need to tune the learning rate for each problem**.
:::


###### 3.4 (10 points)

Fit a neural network with **1 hidden layer** to predict the `y` variable using the `x1` and `x2` predictors.

```{R}
NN1 <- nn_module(
  "NN1",
  initialize = function(p, q1, o) {
    self$hidden1 <- nn_linear(p, q1)
    self$output <- nn_linear(q1, o)
    self$activation <- nn_relu()
  },
  forward = function(x) {
    x %>% 
      self$hidden1() %>%
      self$activation() %>% 
      self$output()
  }
)

model_1 <- NN1(p = 2, q1 = 10, o = 3)

#fit_1 <- model_1 %>%
  #setup(
    #loss = nn_cross_entropy_loss(),
    #optimizer = optim_sgd(model_1$parameters(), lr = 0.1)
  #) %>%
  #set_hparams(
    #p = 2,
    #q1 = 10,
    #o = 3
  #) %>%
  #fit(
    #data = list(
      #x = as.matrix(df %>% select(x1, x2)),
      #y = as.integer(df$y) - 1
    #),
    #epochs = 50,
    #dataloader_options = list(batch_size = 64, shuffle = TRUE),
    #verbose = FALSE
  #)

#predictions_1 <- predict(fit_1, df_test)
#plot_decision_boundary(predictions_1)

```

In order to generate the class predictions, you will need to use the `predict()` function as follows

```{R}
#test_matrix <- df_test %>% select(x1, x2) %>% as.matrix
#fit_1_predictions <- predict(fit_1, test_matrix) %>% 
  ##argmax(2) %>% 
  #as.integer()
```

Plot the results using the `plot_decision_boundary()` function.
```{r}
plot_decision_boundary <- function(predictions) {
  plot(df_test$x1, df_test$x2, col = predictions, pch = 20, cex = 0.5)
  points(df$x1, df$x2, col = df$y + 1, pch = 20)
}

#plot_decision_boundary(fit_1_predictions)


```

---

###### 3.5 (10 points)

Fit a neural network with **0 hidden layers** to predict the `y` variable using the `x1` and `x2` predictors.

```{R}
NN0 <- nn_module(
  initialize = function(p, o) {
    self$output <- nn_linear(p, o)
  },
  forward = function(x) {
    x %>% 
      self$output() 
  }
)

model_0 <- NN0(p = 2, o = 3)

x_matrix <- as.matrix(df %>% select(x1, x2))
y_vector <- as.integer(df$y) - 1

#fit_0 <- model_0 %>%
  #setup(
    #loss = nn_cross_entropy_loss(),
    #optimizer = optim_sgd(model_0$parameters(), lr = 0.01)
  #) %>%
  #set_hparams(
    #p = 2,
    #o = 3
  #) %>%
  #fit(
    #data = list(x = x_matrix, y = y_vector),
    #epochs = 50,
    #dataloader_options = list(batch_size = 64, shuffle = TRUE),
    #verbose = FALSE
  #)


#predictions_0 <- predict(fit_0, df_test)
#plot_decision_boundary(predictions_0)

```

Plot the results using the `plot_decision_boundary()` function.

---


###### 3.6 (10 points)

Fit a neural network with **3 hidden layers** to predict the `y` variable using the `x1` and `x2` predictors.

```{R}
NN3 <- nn_module(
  initialize = function(p, q1, q2, q3, o) {
    self$layer1 <- nn_linear(p, q1)
    self$layer2 <- nn_linear(q1, q2)
    self$layer3 <- nn_linear(q2, q3)
    self$output <- nn_linear(q3, o)
    self$relu <- nn_relu()
  },
  forward = function(x) {
    x %>% 
      self$layer1() %>%
      self$relu() %>% 
      self$layer2() %>%
      self$relu() %>%
      self$layer3() %>%
      self$relu() %>%
      self$output()
  }
)

model_3 <- NN3(p = 2, q1 = 10, q2 = 10, q3 = 10, o = 3)
x_matrix <- as.matrix(df %>% select(x1, x2))
y_vector <- as.integer(df$y) - 1

#fit_3 <- model_3 %>%
  #setup(
    #loss = nn_cross_entropy_loss(),
    #optimizer = optim_sgd(model_3$parameters(), lr = 0.01)
  #) %>%
  #set_hparams(
    #p = 2, q1 = 10, q2 = 10, q3 = 10, o = 3
  #) %>%
  #fit(
    #data = list(x = x_matrix, y = y_vector),
    #epochs = 50,
    #dataloader_options = list(batch_size = 64, shuffle = TRUE),
   # verbose = FALSE
  #)

#predictions_3 <- predict(fit_3, df_test)
#plot_decision_boundary(predictions_3)

```

Plot the results using the `plot_decision_boundary()` function.

---

###### 3.7 (5 points)

What are the differences between the models? How do the decision boundaries change as the number of hidden layers increases?

As the number of hidden layers increases, the decision boundaries become more complex and flexible, allowing for better capture of nonlinear patterns in the data.







---


:::{.hidden unless-format="pdf"}
\pagebreak
:::

<br><br><br><br>
<br><br><br><br>
---



::: {.callout-note collapse="true"}
## Session Information

Print your `R` session information using the following command

```{R}
sessionInfo()
```
:::